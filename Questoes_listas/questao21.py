...
desses carros faz com um litro de combustível. Calcule e mostre:
    ...
    uma tela de exemplo. O disposição das informações deve ser o mais próxima possível ao exemplo. Os dados são fictícios 
...
texto = ""
while(cont < 5):
    print("Veiculo ", cont+1)
    listaCarros.append(input("Nome: "))
    listaKM.append(float(input("KM por litro: ")))
    cont += 1
cont = 0
print("Relatório Final")
while (cont < 5):
    print(cont+1, " - ", listaCarros[cont], "             -     %.1f" % listaKM[cont],
          "  -  %.1f" % (1000/listaKM[cont]), " litros  - R$ %.2f" % ((1000/listaKM[cont])*2.25))
    cont += 1
